...
1.) Run the following checks in parallel:
  ...
  * **redis_queue**: Execute `PING` against the BullMQ Redis connection (BULLMQ_REDIS_URL, from src/interfaces/redis/queues.ts). Pass if it returns `PONG` within 5 seconds, fail otherwise.
