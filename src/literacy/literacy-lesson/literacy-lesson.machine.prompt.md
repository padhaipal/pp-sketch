...
- Sentence state: correct (first/retry) now targets the new `comprehension`
  ...
  (the old `sentence-sentence-complete-correct-*` stids are GONE). First
  failure drills `selectDrillWord` (largest levenshtein ≥ 2, random ties,
  never a word with a conjunct/nukta — i.e. any code point outside
  TEACHABLE_GRAPHEMES); when no teachable word qualifies the sentence
  re-enters itself with stid `sentence-sentence-wrong-retry` and
  sentenceErrors++ (so max two read attempts still holds).
