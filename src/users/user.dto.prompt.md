...
- Students: `PATCH /users/:id/profile` on a STUDENT id (not staff) accepts `name` only (class-view rename from /d; the teacher's link is the credential) — uuid ids only, 404 for unknown/deleted/non-student, 400 if spotlight_message or avatar_seed is sent; returns `{ id, name }`.
